...
|----------|--------|---------|-------------|
| `ALGOCHAINS_TOOL_MODE` | `smart` \| `full` | `smart` | `smart` exposes the documented 181-tool safe set. `full` exposes all 533 tools. Dev/debug only. |
...
```bash
ALGOCHAINS_TOOL_MODE=smart          # Never expose all 533 tools on prod
