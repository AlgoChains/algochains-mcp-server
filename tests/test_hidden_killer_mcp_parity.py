...
def test_server_control_tower_resolver_delegates_to_shared_paths(monkeypatch, tmp_path):
    ...
    desktop_tower = tmp_path / "home" / "trrey" / "algochains-control-tower"
    desktop_tower.mkdir(parents=True)
    ...
    monkeypatch.setattr(
        ...
        (
            tmp_path / "Users" / "treycsa" / "CascadeProjects" / "algochains-control-tower",
            desktop_tower,
        ),
    )

    assert server._default_control_tower() == str(desktop_tower)
