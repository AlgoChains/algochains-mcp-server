...
def test_server_control_tower_resolver_delegates_to_shared_paths(monkeypatch, tmp_path):
    ...
    desktop_root = tmp_path / "home" / "trrey" / "algochains-control-tower"
    ...
    monkeypatch.setattr(paths, "_LEGACY_POSSIBLE_ROOTS", (desktop_root,))

    assert server._default_control_tower() == str(desktop_root)
