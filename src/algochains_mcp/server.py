...
TOOLS = [
    ...
    Tool(name="get_daily_loss_proximity",
         description="Read daily loss proximity guard status: today's P&L vs the $500 hard limit, utilization %, alert/block thresholds (80% alert, 95% block scalpers, MNQ swing exempt), and whether P&L evidence is verified. Reconciles from live bot metrics when state/env is missing or unverified. Returns formatted_line for DAILY-LOSS-PROXIMITY-GUARD Slack posts. Returns DEGRADED when P&L source is unknown instead of fail-open OK.",
         inputSchema={"type": "object", "properties": {}, "required": []},
         annotations=ANNOT_READ_ONLY),
