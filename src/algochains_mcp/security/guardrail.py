...
def _gate_vix(symbol: str, vix: float | None) -> tuple[bool, str]:
    ...
    if vix <= 0:
        ...
        logger.warning(
            ...
            "VIX kill-switch is inactive for symbol=%s",
            symbol,
        )
...
def _gate_daily_loss(
    ...
    """Alert near the limit and block new scalper entries before the hard stop."""
    ...
    if decision.warning and decision.approved:
        logger.warning("GUARDRAIL daily-loss proximity warning: %s", decision.reason)
...
def _gate_stoploss_guard(symbol: str) -> tuple[bool, str]:
    ...
    if state.get("locked"):
        return (
            False,
            f"StoplossGuard: {symbol} locked — too many stops in {_STOPLOSS_WINDOW_H}h window",
        )
...
def _gate_rr(
    entry: float | None, stop: float | None, max_stop_pct: float = 0.05
) -> tuple[bool, str]:
    ...
    if risk_pct > max_stop_pct:
        return (
            False,
            f"Risk/reward: stop is {risk_pct * 100:.1f}% of entry (> {max_stop_pct * 100:.0f}% limit)",
        )
...
def run_guardrail(
    ...
    Args:
        ...
        strategy_type: Strategy family (e.g. scalper, swing). Missing context is
                    treated as scalper-like for fail-safe proximity blocking.
        bot_name:    Optional bot/strategy name used to identify MNQ swing exemptions.
    ...
    for gate_name, gate_fn in run_gates.items():
        ...
        if not ok:
            ...
            if "internal error" not in reason:
                logger.warning(
                    "GUARDRAIL blocked: gate=%s reason=%s symbol=%s", gate_name, reason, symbol
                )
