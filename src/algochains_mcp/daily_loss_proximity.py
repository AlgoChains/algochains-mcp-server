...
def _resolve_daily_pnl(
    ...
    for key in _PNL_KEYS:
        if key in state:
            ...
            if pnl is not None:
                verified = bool(state.get("pnl_verified", state.get("source_verified", True)))
                return pnl, f"state:{key}", verified
    ...
    if isinstance(nested, dict):
        for key in _PNL_KEYS:
            if key in nested:
                ...
                if pnl is not None:
                    verified = bool(nested.get("pnl_verified", nested.get("source_verified", True)))
                    return pnl, f"state:daily.{key}", verified
