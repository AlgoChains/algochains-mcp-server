...
def resolve_tradovate_probe_token(
    control_tower: str | Path,
) -> tuple[str, str]:
    """Return (token, source_label) for live Tradovate probes.

    Priority matches TradovateConnector and live bots:
    tradovate_token_live.txt → state/tradovate_token.json → tradovate_session.json
    → .env TRADOVATE_ACCESS_TOKEN → env TRADOVATE_ACCESS_TOKEN.
    """
    root = Path(control_tower)

    live_path = root / "tradovate_token_live.txt"
    if live_path.exists():
        try:
            token = _strip_token(live_path.read_text())
            if token:
    ...
    json_candidates = (
        ("state/tradovate_token.json", root / "state" / "tradovate_token.json"),
        ("tradovate_session.json", root / "tradovate_session.json"),
    )
    for label, path in json_candidates:
        if not path.exists():
            continue
        data = _read_json(path)
        token = _strip_token(str(_find_value(data, _TOKEN_KEYS) or ""))
        if token:
            return token, label
        ...
        try:
            for raw_line in env_path.read_text().splitlines():
                ...
                if line.startswith("TRADOVATE_ACCESS_TOKEN="):
                    token = _strip_token(line.split("=", 1)[1])
                    if token:
                        return token, ".env:TRADOVATE_ACCESS_TOKEN"
        except OSError:
            pass

    env_token = _strip_token(os.environ.get("TRADOVATE_ACCESS_TOKEN", ""))
    if env_token:
