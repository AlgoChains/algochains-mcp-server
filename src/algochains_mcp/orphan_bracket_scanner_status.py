...
def is_orphan_bracket_scanner_command(command: str) -> bool:
    ...
    for index, token in enumerate(tokens[1:], start=1):
        ...
        if token == "-m":
            ...
            return module_name == "orphan_bracket_scanner" or module_name.endswith(
                ".orphan_bracket_scanner"
            )
...
def format_orphan_bracket_scanner_line(result: dict[str, Any]) -> str:
    """Single-line summary for ORPHAN-BRACKET-SCANNER Slack posts."""
    scan_status = str(result.get("scan_status", "UNKNOWN")).upper()
    prefix = {
        "OK": "[OK]",
        "ORPHAN_ORDERS": "[ORPHAN_ORDERS]",
        "BLOCKED": "[BLOCKED]",
        "UNKNOWN": "[DEGRADED]",
    }.get(scan_status, "[DEGRADED]")

    gate = result.get("gate") or "?"
    swing = result.get("swing") or "?"
    mnq_protect = result.get("mnq_swing_protect") or "?"
    orphan_count = int(result.get("unknown_flat_order_count", 0) or 0)
    cancelled = int(result.get("orphans_cancelled", 0) or 0)

    if scan_status == "OK":
        return (
            f"{prefix} No orphan working orders ({orphan_count} flat-contract legs; "
            f"gate={gate} swing={swing} MNQ_SWING_PROTECT={mnq_protect})"
        )
    if scan_status == "ORPHAN_ORDERS":
        return (
            f"{prefix} {orphan_count} orphan working order(s) on flat contracts "
            f"(cancelled={cancelled}; gate={gate})"
        )
    if scan_status == "BLOCKED":
        return f"{prefix} Orphan scan blocked (gate={gate} swing={swing})"
    return f"{prefix} Orphan scan status unknown — verify scanner state and broker book"
...
def get_orphan_bracket_scanner_status(
    *,
    ...
    payload["formatted_line"] = format_orphan_bracket_scanner_line(payload)
    return payload
